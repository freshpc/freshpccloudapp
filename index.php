...
if (strpos($norm, '/api/') === 0) {
  try{
    ...
    if ($norm==='/api/users' && $method==='GET'){
      api_need_admin();
      $st=$pdo->query("SELECT id,username,email,first_name,last_name,role,activated,phone,
                       COALESCE(NULLIF(full_name,''), CONCAT(COALESCE(first_name,''), ' ', COALESCE(last_name,''))) AS full_name 
